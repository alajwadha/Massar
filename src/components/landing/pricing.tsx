  ...
  const tiers = [
    {
      ...
      price: '199',
      featured: false,
      features: [`2 ${t('feature_paths')}`, t('feature_score'), `10 ${t('feature_targets')}`, t('feature_roadmap')],
    },
    {
      id: 'pro' as const,
      name: t('pro'),
      ...
      price: '499',
      featured: true,
      features: [`5 ${t('feature_paths')}`, t('feature_score'), `40 ${t('feature_targets')}`, t('feature_roadmap')],
    },
  ];
  ...
  return (
    <section id="pricing" className="border-t border-line/70 bg-canvas py-20 sm:py-28">
      <div className="container-page">
            ...
            <Reveal key={tier.id} delay={i * 0.08}>
              ...
              >
                ...
                <Link
                  ...
                  className={cn(
                    'mt-7 inline-flex h-12 w-full items-center justify-center rounded-full text-base font-medium transition-colors duration-200',
                    tier.featured
                      ? 'bg-brand-600 text-white shadow-soft hover:bg-brand-700'
                      : 'border border-line bg-canvas text-ink hover:border-ink/20',
                  )}
                >
                  {t('cta')}
                </Link>
        ...
        <Reveal>
          <p className="mt-6 text-center text-xs text-ink-muted">{t('guarantee')}</p>
        </Reveal>
      </div>
    </section>
  );
