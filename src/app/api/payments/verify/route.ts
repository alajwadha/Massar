...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // TODO (Phase 1, once Supabase is wired): on `ok`, record the payment row
    // keyed by `id` (idempotent — guard against replaying the same paid id),
    // unlock `plan.id` for the buyer, then redirect into onboarding.
    return NextResponse.json({
      ...
      // Surface why a genuinely-paid payment was still rejected — eases
